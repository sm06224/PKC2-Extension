...
| H1 chat-journal | #108 | `dist/pkc2-chat-journal.html` | チャット型セルフメモ(吹き出し UI・Enter 送信・日付セパレータ・#タグ・絵文字)。記録はローカル保持(textlog 互換)。PKC2 直接作成は R5 propose 待ちのため degrade(📋 で日次ログをコピー → 手貼り) |
